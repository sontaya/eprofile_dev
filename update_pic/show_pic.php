...
/*
$sql="SELECT COUNT(*) FROM SDU_BIODATA_TAB";
$stid = @oci_parse($conn, $sql );
@oci_execute($stid);
$row = @oci_fetch_array($stid, OCI_BOTH)
@oci_free_statement($stid);
print ">>><b>".$row["COUNT(*)"]."</b><br>";
*/

$sql="SELECT EMP_ID , BIO_PIC_FILE FROM SDU_BIODATA_TAB WHERE ROWNUM <= 1000";
$stid = @oci_parse($conn, $sql );
@oci_execute($stid);
$data=array();
$i=0;
while($row = @oci_fetch_array($stid, OCI_BOTH)){
	...
	if($row["BIO_PIC_FILE"]!=""){
		print $row["EMP_ID"].":".$row["BIO_PIC_FILE"]."<br>";
	}
	$i++;
}
