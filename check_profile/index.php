 ...
 ?> </p>
        <p><strong>คำชี้แจง</strong><br />
	        <ul>
            	...
            	<li>ขอความร่วมมือบุคลากร ตรวจสอบข้อมูลส่วนบุคคลของท่าน โดยต้องทำการตรวจสอบข้อมูลให้ครบทุกรายการข้อมูล จึงจะสามารถเข้าสู่หน้าจอของระบบการสำรองที่นั่งการอบรมได้</li>
            </ul>
        </p>
        <p class="alert-danger">ขั้นตอนการตรวจสอบข้อมูลส่วนบุคคล (<a href="20151126095000.pdf" target="_blank">pdf</a>)</p>
      </div>
    </div>
<?
...
?>
    <div class="container">
      <!-- Example row of columns -->
        ...
        <li role="presentation" class="active <? if($row_check["PROFILE_STATUS"]){ echo "bg-success"; if($bt_true==2){ $bt_true=2; }else{ $bt_true=1; } }else{ $bt_true=2; } ?>" id="tab_profile"><a href="#profile" aria-controls="profile" role="tab" data-toggle="tab">1. ข้อมูลเบื้องต้น</a></li>
        <li role="presentation" class=" <? if($row_check["ADDRESS_STATUS"]){ echo "bg-success"; if($bt_true==2){ $bt_true=2; }else{ $bt_true=1; } }else{ $bt_true=2; } ?>" id="tab_address"><a href="#address" aria-controls="address" role="tab" data-toggle="tab">2. ที่อยู่ตามทะเบียนบ้าน, ปัจจุบัน</a></li>
        <li role="presentation" class=" <? if($row_check["HISTORY_EDU_STATUS"]){ echo "bg-success"; if($bt_true==2){ $bt_true=2; }else{ $bt_true=1; } }else{ $bt_true=2; } ?>" id="tab_history_edu"><a href="#history_edu" aria-controls="history_edu" role="tab" data-toggle="tab">3. ประวัติการศึกษา</a></li>
        <li role="presentation" class=" <? if($row_check["CURRENT_POSITION_STATUS"]){ echo "bg-success"; if($bt_true==2){ $bt_true=2; }else{ $bt_true=1; } }else{ $bt_true=2; } ?>" id="tab_current_position"><a href="#current_position" aria-controls="current_position" role="tab" data-toggle="tab">4. ตำแหน่งปัจจุบัน</a></li>
        </ul>
  ...
  <div class="row" style="text-align:center">
    ...
    if($bt_true==1){
		?>
        <div class="alert-danger">ข้าพเจ้าขอรับรองว่าข้อมูลข้างต้นเป็นข้อมูลที่ถูกต้องและเป็นความจริงทุกประการ</div>
