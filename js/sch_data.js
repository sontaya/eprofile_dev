...
function del_sch(id){
   ...
   var conf = window.confirm(id +": ข้อมูลชดใช้ทุน และข้อมูลขยายเวลาศึกษาต่อ จะถูกลบไปด้วย ยืนยันที่จะลบรายการนี้");
   if(conf){
      var data = "ID="+id;
...
function del_sch_res(response){
   if(trim_string(response) === "success"){
      load_sch_table();
   }else{
...
function edit_sch(rel){
   ...
   // $("textarea#coun_try").val($('div#coun_try'+rel).html());
	...
	$("input#sch_at_date2").val($('div#sch_at_date2'+rel).html());
