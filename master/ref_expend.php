    ...
    $('#fAdd8').dialog({
      ...
      buttons: {
        'เพิ่ม': function() {
          ...
          $.ajax({
            type: 'POST',
            url : '../master/ref_expend_ajax.php',
            data : {mode : 'add',
              expert_name : expert_name  
            },
            success : function(data){
              $('#tableList').load('../master/ref_expend.php?'+Math.random()+' #tableList');
              alert(data)
            }
          });
          $(this).dialog('close');
  ...
  <div id="mainContainer" >
    ...
    <div id ="tableList">
      ...
      <table width="485" border="1" class="all_table">
        <tr>
          <th width="300" scope="col">รายหัก</th>
          <th width="63" scope="col">แก้ไข</th>
          <th width="51" scope="col">ลบ</th>
        </tr>
        <?php
        // require connected
        $sql = "SELECT * FROM SDU_EXPEND ";
        ...
        while ($rc = oci_fetch_array($st, OCI_ASSOC)) {
          ...
          ?>
          <tr>
            <td><span id="expert_name_id_<?php echo $rc['EXPEND_CODE']; ?>"><?php echo $rc['EXPEND_NAME_TH']; ?></span></td>
            <td align="center"><span onClick="open_f('<?php echo $rc['EXPEND_CODE']; ?>')" style="cursor:pointer"><img src="images/b_edit.png" width="16" height="16" title="Edit" class="vtip" /></span></td>
            <td align="center"><span onClick="open_d('<?php echo $rc['EXPEND_CODE']; ?>')" style="cursor:pointer"><img src="images/b_del.png" width="12" height="12" title="Delete" class="vtip" /></span></td>
          </tr>
          <?php
        }
        ?>
