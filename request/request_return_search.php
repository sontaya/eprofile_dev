...
function con_date($date_str){
		$date=explode(" ",$date_str);
		if(count($date)>1){
			$date2=explode("-",$date[0]);
			return $date2[2]."-".$date2[1]."-".($date2[0]+543);
		}
		else{
			$date2=explode("-",$date_str);
			return $date2[2]."-".$date2[1]."-".($date2[0]+543);
		}
}
...
//print $check_row;
   if ($check_row > 0) {
      ...
      $sql = "SELECT SDU_BIODATA_TAB.*,SDU_NAME_HISTORY.*,SDU_CURRENT_WORK_TAB.*, REQUEST.* FROM (SDU_CURRENT_WORK_TAB INNER JOIN SDU_BIODATA_TAB ON SDU_CURRENT_WORK_TAB.EMP_ID = SDU_BIODATA_TAB.EMP_ID) INNER JOIN  SDU_NAME_HISTORY ON SDU_CURRENT_WORK_TAB.EMP_ID =  SDU_NAME_HISTORY.EMP_ID   INNER JOIN  REQUEST ON SDU_CURRENT_WORK_TAB.EMP_ID =  REQUEST.EMP_ID WHERE $q_where2 ";
     // echo ">".$sql;
      ...
      <table width="716"  border="0" align="center"  bgcolor="#e9e9e9" >
         <tr align="center" class="text_th">
            <td width="52" class="text_tr">sssลำดับ</td>
   ...
   if ($check_row > 0) {
      //$sql = "SELECT SDU_BIODATA_TAB.*,SDU_CURRENT_WORK_TAB.EMP_ID,SDU_CURRENT_WORK_TAB.CWK_STATUS FROM SDU_BIODATA_TAB,SDU_CURRENT_WORK_TAB WHERE $q_where ORDER BY BIO_FNAME_TH ASC  ";
	    $sql = "SELECT SDU_BIODATA_TAB.*,SDU_CURRENT_WORK_TAB.EMP_ID,SDU_CURRENT_WORK_TAB.CWK_STATUS,REQUEST.* FROM SDU_BIODATA_TAB,SDU_CURRENT_WORK_TAB  INNER JOIN  REQUEST ON SDU_CURRENT_WORK_TAB.EMP_ID =  REQUEST.EMP_ID WHERE $q_where ORDER BY RE_ID DESC  ";
//echo $sql;
      $stid = oci_parse($conn, $sql);
      oci_execute($stid);
      ?>
         ...
         <table width="716"  border="0" align="center"  bgcolor="#e9e9e9" >
            <tr align="center" class="text_th">
               ...
               <td width="150" class="text_tr">ชื่อ - นามสกุล</td>
               <td width="130" class="text_tr">เลขบัตรบุคลากร </td>
               <td width="150" class="text_tr">วันที่แจ้งเรื่องคำร้อง</td>
               <td width="50" class="text_tr">สถานะ</td>
               <td width="81" class="text_tr">การร้องขอ</td>
      <?php if ($_SESSION['USER_TYPE'] == 'hr' || $_SESSION['USER_TYPE'] == 'admin') { ?>
               <td class="text_tr">คำร้อง</td>
               <?php } ?>
            </tr>
               <?
               while (($row = oci_fetch_array($stid, OCI_BOTH))) {
				  ...
				  $type="";
				  if($row["RE_TYPE"]=="1"){
				  	$type="ข้อมูลไม่ทันสมัย";
				  }
				  else if($row["RE_TYPE"]=="2"){
				  	$type="ข้อมูลผิด";
				  }
                  ?>
               <tr align="center" height="22" valign="top">
                  ...
                  <td align="center" class="text_td"><?= $row["EMP_ID"] ?></td>
                  <td align="center" class="text_td"><?=con_date($row["CREATE_DATE"])?></td>
                  <td align="center" class="text_td"><?=$status?></td>
                  <!--<td align="center" class="text_td"><img src="../images/b_edit.png" height="15" border="0" style="cursor:pointer" onclick="edit_('<?= $row["EMP_ID"] ?>');" title="Edit" class="vtip" /></td> -->
                  <td align="center" class="text_td"><?=$type?></td>
         <?php if ($_SESSION['USER_TYPE'] == 'hr' || $_SESSION['USER_TYPE'] == 'admin') { ?>
                     <td align="center" class="text_td"><span style="cursor:pointer" onclick="red_detail('<?php echo $row['RE_ID']; ?>')"><img src="../images/view.png" border="0" height="15" /></span></td>
                  <?php } ?>
               </tr>
                  <?
               }
         ...
         <br />
        	สถานะ เปิด = คำร้องขอใหม่&nbsp;&nbsp;&nbsp;สถานะ ยกเลิก = คำร้องนี้ ไม่แก้ไขให้ &nbsp;&nbsp;&nbsp; สถานะ ปิด = คำร้องนี้จัดการไปแล้ว 
