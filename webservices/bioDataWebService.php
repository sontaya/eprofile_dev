...
function current_work_tab($empId,$conn){
	$sql = "SELECT SDU_CURRENT_WORK_TAB.*, SDU_REF_STATUS_EXT.STATUS_NAME AS CWK_STATUS_NAME, SDU_REF_SITE.NAME_SITE AS CWK_DSU_EDU_CENTER_NAME
            FROM SDU_CURRENT_WORK_TAB , SDU_REF_STATUS_EXT , SDU_REF_SITE
            WHERE SDU_REF_STATUS_EXT.STATUS_ID = SDU_CURRENT_WORK_TAB.CWK_STATUS 
                    AND SDU_CURRENT_WORK_TAB.CWK_DSU_EDU_CENTER = SDU_REF_SITE.CODE_SITE(+)  
					AND SDU_CURRENT_WORK_TAB.EMP_ID = '".$empId."'";
...
function department_sub_tab($fac,$dep,$conn){
	$sql="SELECT * FROM SDU_REF_DEPARTMENT_SUB 
					WHERE CODE_FACULTY = '".$fac."' AND CODE_DEPARTMENT_SECTION = '".$dep."'";
	$stid = oci_parse($conn,$sql);
	oci_execute($stid);
	$row = oci_fetch_array($stid, OCI_BOTH);
	return $row;
...
function bioData($arr=array()){
		...
		$dep_sec = department_sub_tab($work["CWK_MUA_MAIN"],$work["CWK_MUA_SUBMAIN"],$conn);
		//$edu=new stdClass;
		...
		$return =array(
			"return"=>array(
				array(
					...
					"CWK_MUA_SUBMAIN_NAME"=>chk_null($dep_sec["NAME_DEPARTMENT_SECTION"]),
					"CWK_DSU_EDU_CENTER"=>chk_null($work["CWK_DSU_EDU_CENTER"]),
					"CWK_DSU_EDU_CENTER_NAME"=>chk_null($work["CWK_DSU_EDU_CENTER_NAME"]),
...
function ByCriteria($arr=array()){
	...
	*/
	...
	if($input["FNAME_EN"]!=""){
			$sql.=" AND LOWER(SDU_BIODATA_TAB.BIO_FNAME_EN) LIKE LOWER('".$input["FNAME_EN"]."%')";		
	}
	if($input["LNAME_EN"]!=""){

			$sql.=" AND LOWER(SDU_BIODATA_TAB.BIO_LNAME_EN) LIKE LOWER('".$input["LNAME_EN"]."%')";
	}
	...
	if($input["PERSON_TYPE"]!="" and $input["PERSON_TYPE"]!="0"){
		...
		*/
		...
		if($input["PERSON_TYPE"]=="3"){
			$sql.=" AND (SDU_CURRENT_WORK_TAB.CWK_MUA_EMP_TYPE NOT IN '10') AND (SDU_CURRENT_WORK_TAB.CWK_MUA_EMP_SUBTYPE='".$set_type[$input["PERSON_TYPE"]][0]."') ";
			...
			$sql.=" AND SDU_CURRENT_WORK_TAB.CWK_MUA_EMP_TYPE = '".$set_type[$input["PERSON_TYPE"]][0]."'";
		}
	...
	if($input["CWK_MUA_MAIN"]!=""){
		...
		$strFaculty = $input["CWK_MUA_MAIN"];
		$arrFaculty = explode(",",$strFaculty);
		$str = "'";
		foreach	($arrFaculty as $key => $value) {
			$str = $str.$value."','";
		}
	
		$faculty_string =  substr($str,0,strlen($str)-2);
		
		$sql.=" AND SDU_CURRENT_WORK_TAB.CWK_MUA_MAIN IN ( ".$faculty_string." ) ";
		$join=1;
	}
	if($input["CWK_MUA_SUBMAIN"]!=""){
		
		$strDepartment = $input["CWK_MUA_SUBMAIN"];
		$arrDepartment = explode(",",$strDepartment);
		$str = "'";
		foreach	($arrDepartment as $key => $value) {
			$str = $str.$value."','";
		}
	
		$department_string =  substr($str,0,strlen($str)-2);
		
		$sql.=" AND SDU_CURRENT_WORK_TAB.CWK_MUA_SUBMAIN IN ( ".$department_string." ) ";
		$join=1;
	}
	
	if($input["CWK_DSU_EDU_CENTER"] != ""){
	
		$strSite = $input["CWK_DSU_EDU_CENTER"];
			$arrSite = explode(",",$strSite);
			$str = "'";
			foreach	($arrSite as $key => $value) {
				$str = $str.$value."','";
			}
	
		$site_string =  substr($str,0,strlen($str)-2);
		
		$sql.=" AND SDU_CURRENT_WORK_TAB.CWK_DSU_EDU_CENTER IN ( ".$site_string." )";
		$join=1;
	...
	while($row = oci_fetch_array($stid,OCI_BOTH)){
		...
		$text[15] = new SoapVar(chk_null($work["CWK_DSU_EDU_CENTER"]),XSD_STRING,null,null,"CWK_DSU_EDU_CENTER");
		$text[16] = new SoapVar(chk_null($work["CWK_DSU_EDU_CENTER_NAME"]),XSD_STRING,null,null,"CWK_DSU_EDU_CENTER_NAME");
		$text[17] = new SoapVar(chk_null($row["BIO_MOBILE_1"]),XSD_STRING,null,null,"BIO_MOBILE_1");
		$text[18] = new SoapVar(chk_null($row["BIO_MOBILE_2"]),XSD_STRING,null,null,"BIO_MOBILE_2");
		$text[19] = new SoapVar(chk_null($row["BIO_EMAIL1"]),XSD_STRING,null,null,"BIO_EMAIL1");
		$text[20] = new SoapVar(chk_null($row["BIO_EMAIL2"]),XSD_STRING,null,null,"BIO_EMAIL2");
		$text[21] = new SoapVar(chk_null($work["CWK_MUA_MAIN"]),XSD_STRING,null,null,"CWK_MUA_MAIN");
		$text[22] = new SoapVar(chk_null($department[$work["CWK_MUA_MAIN"]]["NAME_FACULTY"]),XSD_STRING,null,null,"CWK_MUA_MAIN_NAME");
		$text[23] = new SoapVar(chk_null($dep_sec["NAME_DEPARTMENT_SECTION"]),XSD_STRING,null,null,"CWK_MUA_SUBMAIN_NAME");
		$text[24] = new SoapVar(chk_null($work["CWK_STATUS_NAME"]),XSD_STRING,null,null,"CWK_STATUS_NAME");
		unset($edu_xml);
		$edu = sdu_education_tab($row["EMP_ID"],$conn);
		foreach($edu as $value){
