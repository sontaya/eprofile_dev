...
$numrow = $db->count_row(TB_POSITION_SUB_TAB," WHERE EMP_ID = '$emp_id' AND VP_TYPE = '$vp_type' ",$conn);

	if($numrow == 0 ){
		$result=$db->add_db(TB_POSITION_SUB_TAB,array(
 											  "EMP_ID"=>"$emp_id",
											  "VP_TYPE"=>"$vp_type",
		...
		$result=$db->update_db(TB_POSITION_SUB_TAB,array(
											 "VP_SUB_TYPE"=>"$vp_sub_type",
											  "VP_METHOD"=>"$vp_method",
											  "VP_BY"=>"$vp_by",
											  "VP_UNIVERSITY"=>"$vp_university",
	...
	$db->closedb($conn);	
		if($result){
			...
			window.top.load_pos_sub_list(<?=$vp_type?>);
			</script>
			<?
