...
while($row_all_person1 = oci_fetch_array($stid_all_person1, OCI_BOTH)){
	...
	if($all_person_temp1 > 0 ){
		...
		while($row_query1 = oci_fetch_array($stid_query1, OCI_BOTH)){
			$num1_all_temp11 =  $db->count_row(TB_SCHOLAR_TAB," WHERE  EMP_ID = '".$row_query1["EMP_ID"]."' AND  (SCH_EDU_LEVEL = '2' OR SCH_EDU_LEVEL = '3')");
			if($num1_all_temp11 > 0 ) $num1_all11++;//เอก
			
			$num2_all_temp11 =  $db->count_row(TB_SCHOLAR_TAB," WHERE  EMP_ID = '".$row_query1["EMP_ID"]."' AND  SCH_EDU_LEVEL = '1' ");
...
while($row_all_person1 = oci_fetch_array($stid_all_person1, OCI_BOTH)){
	...
	if($all_person_temp1 > 0 ){
		...
		while($row_query1 = oci_fetch_array($stid_query1, OCI_BOTH)){
			...
			$num2_all_temp14 =  $db->count_row(TB_SCHOLAR_TAB," WHERE  EMP_ID = '".$row_query1["EMP_ID"]."' AND  SCH_EDU_LEVEL = '1' ");
		...
		while($row_query2 = oci_fetch_array($stid_query2, OCI_BOTH)){
			...
			$num2_all_temp34 =  $db->count_row(TB_SCHOLAR_TAB," WHERE  EMP_ID = '".$row_query2["EMP_ID"]."' AND  SCH_EDU_LEVEL = '1' ");
...
while($row_all_person1 = oci_fetch_array($stid_all_person1, OCI_BOTH)){
	...
	if($all_person_temp1 > 0 ){
		...
		while($row_query1 = oci_fetch_array($stid_query1, OCI_BOTH)){
			...
			$num2_all_temp18 =  $db->count_row(TB_SCHOLAR_TAB," WHERE  EMP_ID = '".$row_query1["EMP_ID"]."' AND  SCH_EDU_LEVEL = '1' ");
...
<title>ข้อมูลบุคลากรสายสนับสนุนการมหาวิทยาลัยราชภัฎสวนดุสิต</title>
<style type="text/css">
body,td,th {
...
<div align="center"> <input type="button" value="Export to Excel" onclick="window.location = 'scholar_report2_excel.php?excel=1&date1=<?=$_REQUEST["date1"]?>&date2=<?=$_REQUEST["date2"]?>'"/></div>
