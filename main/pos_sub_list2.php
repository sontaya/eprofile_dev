...
$sql = "SELECT * FROM  " . TB_POSITION_SUB_TAB . "  WHERE  EMP_ID= '" . $_SESSION["EMP_ID"] . "' AND VP_TYPE = '2'";
...
<table width="670" border="0" cellspacing="4" cellpadding="4">
   ...
   <tr>
      ...
      <td align="left" class="form_text"><input type="text" name="vp_university2" id="vp_university2" style="width: 300px;" class="input_text" value="<?= $row2["VP_UNIVERSITY"] ?>"<?php if ($_SESSION['USER_TYPE'] == 'user' || $_SESSION['USER_TYPE'] == 'chief') { ?> readonly="readonly"<?php } ?>/></td>
   </tr>
   <tr>
      <td align="right" class="form_text"> * สาขาวิชาที่ขอกำหนดตำแหน่ง :</td>
      <td align="left"><input type="text" name="vp_professional_major2" id="vp_professional_major2" style="width: 300px;" class="input_text" value="<?= $row2["VP_PROFESSIONAL_MAJOR"] ?>"<?php if ($_SESSION['USER_TYPE'] == 'user' || $_SESSION['USER_TYPE'] == 'chief') { ?> readonly="readonly"<?php } ?>/></td>
   </tr>
   <tr>
      <td align="right" > * วันที่ได้รับอนุมัติแต่งตั้ง :</td>
      <td align="left"><input  type="text" name="vp_date2" id="vp_date2" style="width:80px;" class="input_text" value="<?= change_date_thai($row2["VP_DATE"]) ?>"<?php if ($_SESSION['USER_TYPE'] == 'user' || $_SESSION['USER_TYPE'] == 'chief') { ?> readonly="readonly"<?php } ?>/> <img src="../images/vcalendar.png" align="absmiddle"<?php if ($_SESSION['USER_TYPE'] != 'user' && $_SESSION['USER_TYPE'] != 'chief') { ?> onclick="showCalendar('vp_date2','YYYY-MM-DD')"<?php } ?>  style="cursor:pointer"/></td>
   </tr>
   <tr>
      <td align="right" > คำสั่งที่ :</td>
      <td align="left"><input type="text" name="vp_order2" id="vp_order2" style="width: 80px;" class="input_text"  maxlength="2" value="<?= $row2["VP_ORDER"] ?>"<?php if ($_SESSION['USER_TYPE'] == 'user' || $_SESSION['USER_TYPE'] == 'chief') { ?> readonly="readonly"<?php } ?>/>   ณ วันที่ <input  type="text" name="vp_order_date2" id="vp_order_date2" style="width: 80px;" class="input_text" value="<?= change_date_thai($row2["VP_ORDER_DATE"]) ?>"<?php if ($_SESSION['USER_TYPE'] == 'user' || $_SESSION['USER_TYPE'] == 'chief') { ?> readonly="readonly"<?php } ?>/> <img src="../images/vcalendar.png" align="absmiddle"<?php if ($_SESSION['USER_TYPE'] != 'user' && $_SESSION['USER_TYPE'] != 'chief') { ?> onclick="showCalendar('vp_order_date2','YYYY-MM-DD')"<?php } ?>  style="cursor:pointer"/></td>
   </tr>
   <tr>
      <td align="right" > * มติที่ประชุมสภามหาวิทยาลัย/สถาบันการศึกษา ครั้งที่ :</td>
      <td align="left"><input type="text" name="vp_mati_12" id="vp_mati_12" style="width: 20px;" class="input_text"  maxlength="2" value="<?= $row2["VP_MATI_1"] ?>"<?php if ($_SESSION['USER_TYPE'] == 'user' || $_SESSION['USER_TYPE'] == 'chief') { ?> readonly="readonly"<?php } ?>/> วันที่  <input type="text" name="vp_mati_22" id="vp_mati_22" style="width: 80px;" class="input_text" value="<?= change_date_thai($row2["VP_MATI_2"]) ?>"<?php if ($_SESSION['USER_TYPE'] == 'user' || $_SESSION['USER_TYPE'] == 'chief') { ?> readonly="readonly"<?php } ?>/> <img src="../images/vcalendar.png" align="absmiddle"<?php if ($_SESSION['USER_TYPE'] != 'user' && $_SESSION['USER_TYPE'] != 'chief') { ?> onclick="showCalendar('vp_mati_22','YYYY-MM-DD')"<?php } ?>  style="cursor:pointer"/></td>
   </tr>
</table>
