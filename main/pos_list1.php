...
$sql = "SELECT * FROM  " . TB_VCHARKARN_POSITION_TAB . "  WHERE  EMP_ID= '" . $_SESSION["EMP_ID"] . "' AND VP_TYPE = '1' ";
...
<table width="670" border="0" cellspacing="4" cellpadding="4">
  <tr>
    ...
    <td width="358"    align="left">
      <input type="radio" id="vp_sub_type1" name="vp_sub_type1" value="1" <? if ($row1["VP_SUB_TYPE"] == "1") { echo "checked='checked'"; } ?><?php if ($_SESSION['USER_TYPE'] == 'user' || $_SESSION['USER_TYPE'] == 'chief') { ?> onfocus="myDisable(this);"<?php } ?> /> ผู้ช่วยศาสตราจารย์
      <input type="radio" id="vp_sub_type1" name="vp_sub_type1" value="2" <? if ($row1["VP_SUB_TYPE"] == "2") { echo "checked='checked'"; } ?><?php if ($_SESSION['USER_TYPE'] == 'user' || $_SESSION['USER_TYPE'] == 'chief') { ?> onfocus="myDisable(this);"<?php } ?>/> ผู้ช่วยศาสตราจารย์พิเศษ
    </td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td align="left" class="form_text"><input onkeydown="form_enter_tab('vp_professional_major1');" type="text" name="vp_university1" id="vp_university1" style="width: 300px;" class="input_text" value="<?= $row1["VP_UNIVERSITY"] ?>"<?php if ($_SESSION['USER_TYPE'] == 'user' || $_SESSION['USER_TYPE'] == 'chief') { ?> readonly="readonly"<?php } ?>/></td>
  </tr>
  <tr>
    <td align="right" class="form_text"> * สาขาวิชาที่ขอกำหนดตำแหน่ง :</td>
    <td align="left"><input onkeydown="form_enter_tab('vp_date1');" type="text" name="vp_professional_major1" id="vp_professional_major1" style="width: 300px;" class="input_text" value="<?= $row1["VP_PROFESSIONAL_MAJOR"] ?>"<?php if ($_SESSION['USER_TYPE'] == 'user' || $_SESSION['USER_TYPE'] == 'chief') { ?> readonly="readonly"<?php } ?>/></td>
  </tr>
  <tr>
    <td align="right" > * วันที่ได้รับอนุมัติแต่งตั้ง :</td>
    <td align="left"><input onkeydown="form_enter_tab('vp_order1');"  type="text" name="vp_date1" id="vp_date1" style="width:80px;" class="input_text" value="<?= change_date_thai($row1["VP_DATE"]) ?>"<?php if ($_SESSION['USER_TYPE'] == 'user' || $_SESSION['USER_TYPE'] == 'chief') { ?> readonly="readonly"<?php } ?>/> <img src="../images/vcalendar.png" align="absmiddle"<?php if ($_SESSION['USER_TYPE'] != 'user' && $_SESSION['USER_TYPE'] != 'chief') { ?> onclick="showCalendar('vp_date1','YYYY-MM-DD')"<?php } ?>  style="cursor:pointer"/></td>
  </tr>
  <tr>
    <td align="right" > คำสั่งที่ :</td>
    <td align="left"><input onkeydown="form_enter_tab('vp_order_date1');" type="text" name="vp_order1" id="vp_order1" style="width: 80px;" class="input_text"   value="<?= $row1["VP_ORDER"] ?>"<?php if ($_SESSION['USER_TYPE'] == 'user' || $_SESSION['USER_TYPE'] == 'chief') { ?> readonly="readonly"<?php } ?>/>   ณ วันที่ <input onkeydown="form_enter_tab('vp_mati_11');"  type="text" name="vp_order_date1" id="vp_order_date1" style="width: 80px;" class="input_text" value="<?= change_date_thai($row1["VP_ORDER_DATE"]) ?>"<?php if ($_SESSION['USER_TYPE'] == 'user' || $_SESSION['USER_TYPE'] == 'chief') { ?> readonly="readonly"<?php } ?>/> <img src="../images/vcalendar.png" align="absmiddle"<?php if ($_SESSION['USER_TYPE'] != 'user' && $_SESSION['USER_TYPE'] != 'chief') { ?> onclick="showCalendar('vp_order_date1','YYYY-MM-DD')"<?php } ?>  style="cursor:pointer"/></td>
  </tr>
  <tr>
    <td align="right" > * มติที่ประชุมสภามหาวิทยาลัย/สถาบันการศึกษา ครั้งที่ :</td>
    <td align="left"><input onkeydown="form_enter_tab('vp_mati_21');" type="text" name="vp_mati_11" id="vp_mati_11" style="width: 70px;" class="input_text"  maxlength="50" value="<?= $row1["VP_MATI_1"] ?>"<?php if ($_SESSION['USER_TYPE'] == 'user' || $_SESSION['USER_TYPE'] == 'chief') { ?> readonly="readonly"<?php } ?>/>   วันที่ <input type="text" name="vp_mati_21" id="vp_mati_21" style="width: 80px;" class="input_text" value="<?= change_date_thai($row1["VP_MATI_2"]) ?>"<?php if ($_SESSION['USER_TYPE'] == 'user' || $_SESSION['USER_TYPE'] == 'chief') { ?> readonly="readonly"<?php } ?>/> <img src="../images/vcalendar.png" align="absmiddle"<?php if ($_SESSION['USER_TYPE'] != 'user' && $_SESSION['USER_TYPE'] != 'chief') { ?> onclick="showCalendar('vp_mati_21','YYYY-MM-DD')"<?php } ?>  style="cursor:pointer"/></td>
  </tr>
</table>
