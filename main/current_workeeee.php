...
$sql = "SELECT * FROM  ".TB_CURRENT_WORK_TAB."  WHERE  EMP_ID = '".$_SESSION["EMP_ID"]."' and CWK_POS_STATUS='2'"; 
...
function check_data(){
		...
		/*if(!Checkfiles($("input#cwk_teacher_file"))){
				$("input#cwk_teacher_file").val("");
				$("#Valid_edu_file").dialog('open');
				return false;
		}*/
...
<table cellpadding="0" cellspacing="0" align="center" width="758" border="0">
    ...
    <tr><td >
    <div id="current_work_list" align="center" class="data_details_list">
      <? include "current_work_data_table.php";?>
    </div>
    <div align="center"  id="toggle_form"><?php if( $_SESSION['USER_TYPE'] != 'chief') { ?><!--<img src="../images/add.png" onclick="toggle_form('current_work','current_work_id')" style="cursor:pointer"/>--><?php } ?>&nbsp;</div>
      <div id="data_form"> 
      <img src="../images/bg_d.png" style="margin-left:10px;" />

<table  cellspacing="0" cellpadding="0" align="center" border="0">
 <tr>
    <td><form id="current_work" name="current_work" enctype="multipart/form-data" method="post" action="current_work_data_save.php" target="upload_target">
    ...
    <input type="hidden" id="current_work_id" name="current_work_id" value=""/>
     <input type="hidden" id="ch_id" name="ch_id" value=""/>
      <input type="hidden" id="cwk_id" name="cwk_id" value=""/>
     ...
     <tr>
      ...
      <tr >
		...
		?>
        <div id="quit"  align="left" style="padding-left:145px;<?=$style?>">
       ...
       &nbsp;&nbsp;<input type="text" id="cwk_quit_date" name="cwk_quit_date" class="input_text" style="width: 80px; " value="<?=change_date_thai($row["CWK_QUIT_DATE"])?>"/> 
        ...
        เหตุผลที่ออก : 
        &nbsp;&nbsp;<input type="text" id="cwk_quit_reason" name="cwk_quit_reason" class="input_text" style="width: 200px; " value="<?=$row["CWK_QUIT_REASON"]?>"/>
        </div>
       ...
       <!-- </select> <input type="button" value="ดูประวัติตำแหน่ง/ย้ายสังกัด" onclick="work_history('<?=$_SESSION["EMP_ID"]?>')" />-->
        </td>
      </tr>
      <tr>
        <td width="223" align="right" class="form_text">* ประเภทบุคลากร<!--(สกอ.)--> :</td>
...
<!------------------------------------->
 <tr>
        <td align="right" class="form_text">วันที่บรรจุเป็นพนักงานหหาวิทยาลัย :</td>
        <td align="left">
       <input type="text" id="date_m" name="date_m" class="input_text" style="width: 80px; " value="<?=change_date_thai($row["DATE_M"])?>"/> 
        <img src="../images/vcalendar.png" align="absmiddle" onclick="showCalendar('date_m','YYYY-MM-DD')"  style="cursor:pointer"/> <br />
        </td>
    </tr>
	<tr>
...
<!--
    <td width="42%" align="right" class="form_text" >ศูนย์การศึกษา(มสด.) : </td>
	...
	?>
     <select name="cwk_dsu_edu_center" id="cwk_dsu_edu_center" style="width:150px;" >
	<?/*$option_ref_site*/?>
            </select>
-->
    </td>
  ...
  <tr>
	...
	?>
	  ...
	  <tr>
        ...
        <td align="left">
         <input type="text" name="position_code" id="position_code" style="width: 100px; " class="input_text" />
		</td>
	  </tr>
<!-------------------------------------->
...
<!-------------------------------------->
  ...
  <tr>
    ...
    <!--
    <td width="42%" align="right" class="form_text">ตำแหน่งบริหาร<!--(สกอ.)--><!--:</td>
    <td width="39%" align="left"><?
	/*
	...
	*/?>
      <!-- <select name="cwk_mua_mpos" id="cwk_mua_mpos" class="widthFix2">
        <?=$option_mua_mpos?>
      </select></td>
      -->
  </tr>
...
<tr id="executives_data1" <? if($cwk_mua_mpos=="00" or $cwk_mua_mpos==""){ ?>style="display:none;" <? } ?> bgcolor="#E8E3F9">
 ...
 <tr id="executives_data2" <? if($cwk_mua_mpos=="00" or $cwk_mua_mpos==""){ ?>style="display:none;" <? } ?> bgcolor="#E8E3F9">
   ...
   <tr id="executives2" <? if($cwk_mua_mpos=="00" or $cwk_mua_mpos==""){ ?>style="display:none;" <? } ?> bgcolor="#D7EBFF">
	...
	?>
      <select name="cwk_position2" id="cwk_position2" style="width:480px;"  onchange="open_executives_data(this);">
        <?=$option_mua_mpos?>
      </select>
   		 		</td>
  			</tr>
 ...
 <tr id="executives_data4" <? if($cwk_mua_mpos=="00" or $cwk_mua_mpos==""){ ?>style="display:none;" <? } ?> bgcolor="#D7EBFF" >
	...
	?>
             <select name="cwk_mua_submain_data2" id="cwk_mua_submain_data2"  style="width:480px;" >
               <?=$option_ref_department_sub?>
             </select>
                        </div>
   		 		</td>
   ...
   <tr id="executives3" <? if($cwk_mua_mpos=="00" or $cwk_mua_mpos==""){ ?>style="display:none;" <? } ?> bgcolor="#FBDBD0">
	...
	?>
      <select name="cwk_position3" id="cwk_position3" style="width:480px;"  onchange="open_executives_data(this);">
        <?=$option_mua_mpos?>
      </select>
   		 		</td>
  			</tr>
 ...
 <tr id="executives_data6" <? if($cwk_mua_mpos=="00" or $cwk_mua_mpos==""){ ?>style="display:none;" <? } ?> bgcolor="#FBDBD0">
	...
	?>
             <select name="cwk_mua_submain_data3" id="cwk_mua_submain_data3"  style="width:480px;" >
               <?=$option_ref_department_sub?>
             </select>
                        </div>
   		 		</td>
      ...
      <tr>
        ...
        <img src="../images/vcalendar.png" align="absmiddle" onclick="showCalendar('cwk_start_work_date','YYYY-MM-DD')"  style="cursor:pointer"/>&nbsp; &nbsp;วันครบทดลองปฏิบัติงาน : <input type="text"  name="cwk_end_work_date" id="cwk_end_work_date" style="width: 80px; " class="input_text" value="<?=change_date_thai($row["CWK_END_WORK_DATE"])?>"/> 
	...
	$q4=substr($row["CWK_END_WORK"], 3, 2);
	 ?>
      <tr>
        ...
        <input type="text" name="cwk_start_work_hour" id="cwk_start_work_hour" style="width: 25px; " class="input_text" value="<?=$q1;?>"/>  : 
        <input type="text" name="cwk_start_work_min" id="cwk_start_work_min" style="width: 25px; " class="input_text" value="<?=$q2;?>"/> น.
         &nbsp;&nbsp;&nbsp; ถึงเวลา &nbsp;&nbsp;&nbsp;
        <input type="text" name="cwk_end_work_hour" id="cwk_end_work_hour" style="width: 25px; " class="input_text" value="<?=$q3;?>"/>  : 
        <input type="text" name="cwk_end_work_min" id="cwk_end_work_min" style="width: 25px; " class="input_text" value="<?=$q4;?>" /> น.
        </td>
      </tr>
      <tr>
        ...
        <img src="../images/vcalendar.png" align="absmiddle" onclick="showCalendar('cwk_start_teach_date','YYYY-MM-DD')"  style="cursor:pointer"/>&nbsp; &nbsp; คำสั่งที่ <input type="text" id="cwk_order1" name="cwk_order1" class="input_text" style="width: 90px" value="<?=$row["CWK_ORDER1"]?>" />
        <label> สั่ง ณ :  </label>
        <input type="text"  name="cwk_teach_order" id="cwk_teach_order" style="width: 120px; " value="<?=$row['CWK_TEACH_ORDER']?>"  class="input_text" />
        </td>
      </tr>
      <tr>
        ...
        <img src="../images/vcalendar.png" align="absmiddle" onclick="showCalendar('cwk_promote_date','YYYY-MM-DD')"  style="cursor:pointer"/>&nbsp; &nbsp; คำสั่งที่ <input type="text" id="cwk_order2" name="cwk_order2" class="input_text" style="width: 90px" value="<?=$row["CWK_ORDER2"]?>"/>
        <label> สั่ง ณ :  </label>
        <input type="text"  name="cwk_promote_order" id="cwk_promote_order" style="width: 120px; " value="<?=$row['CWK_PROMOTE_ORDER']?>"  class="input_text" />
        </td>
      </tr>
      <tr>
        <td align="right" >ทำงานวันหยุด : </td>
        <td align="left"><input type="checkbox" id="cwk_sat" name="cwk_sat" value="1" <? if($row["CWK_SAT"] == "1"){ echo "checked = 'checked'";}?> /> วันเสาร์ <input type="checkbox" id="cwk_sun" name="cwk_sun" value="1" <? if($row["CWK_SUN"] == "1"){ echo "checked = 'checked'";}?>/> วันอาทิตย์</td>
      </tr>
      <tr>
        <td align="right" >&nbsp;</td>
        <td align="left">&nbsp;</td>
      ...
      <tr>
        <td height="140" colspan="2" align="center" valign="top" >
        <table width="519" border="0" cellspacing="3" cellpadding="3" align="center">
  <tr>
    <td width="507" class="form_text">เงินเดือน 
      : 
<?

?>
      <input type="text" id="salary" name="salary" class="input_text" value="<?=number_format($salary,2)?>" style="text-align:right"<?php if($_SESSION['USER_TYPE'] != 'admin') { ?>  readonly="readonly"<?php } ?> />
      บาท    &nbsp;   &nbsp;ประกอบด้วย</td>
  </tr>
</table>
<?
...
<table width="726" border="0" cellspacing="3" cellpadding="3" align="right">
  <tr>
    ...
    <td width="130" align="left" class="form_text"><select id="cwk_budget1" name="cwk_budget1" style="width: 130px"><?=$option[1]?></select></td>
  ...
  <tr>
    ...
    <td align="left"  class="form_text"><select id="cwk_budget2" name="cwk_budget2" style="width: 130px"><?=$option[2]?></select></td>
  ...
  <tr>
    ...
    <td align="left"  class="form_text"><select id="cwk_budget3" name="cwk_budget3" style="width: 130px"><?=$option[3]?></select></td>
    <td align="center"  class="form_text"><span class="form_text">จำนวนเงิน</span></td>
  ...
  <div align="left" style="padding-left: 90px; font-size:16px">เงินพิเศษ</div><br />      
  <div id="ex_salary_list" align="center">
  <? include "ex_salary_list.php";?>
</div><br />
<span  id="morefile"></span><br /><div style="padding-left: 140px" align="left"><input type="button" id="addfile" value="เพิ่ม" /></div><br />
        </td>
        </tr>
                <tr>
           <td width="223" align="right" class="form_text">ประเภทระยะเวลาการว่าจ้าง :</td>
           <td width="507" align="left">
             <select name="cwk_salary_time_type" id="cwk_salary_time_type" style="width: 500px">
             ...
             <?
				...
				while(($row_time = oci_fetch_array($stid_time, OCI_BOTH))){
			 ?>
               <option value="<?=$row_time["TIME_CONTACT_ID"] ?>"><?=$row_time["TIME_CONTACT_NAME"] ?></option>
               <? } ?>
               </select>
             </td>
         </tr>
      ...
      <tr>
        ...
        <td align="left">
        <select name="cwk_edu_group1" id="cwk_edu_group1" style="width: 500px">
           <?=$option_teach[1]?>
            </select>
        </td>
      </tr>
      <tr>
        ...
        <td align="left">
         <select name="cwk_edu_group2" id="cwk_edu_group2" style="width: 500px">
            <?=$option_teach[2]?>
            </select>
        </td>
      </tr>
      <tr>
        ...
        <td align="left">
         <select name="cwk_edu_group3" id="cwk_edu_group3" style="width: 500px">
            <?=$option_teach[3]?>
            </select>
        </td>
      </tr>
      <tr>
        ...
        <td align="left" style="color:#663; font-size:11px" valign="middle">
        <div id="show_upload1">
        <?
		if($row["CWK_TEACHER_FILE"] == ""){
		?>
        <input type="file" name="cwk_teacher_file" id="cwk_teacher_file" class="file_upload"/> เฉพาะ .jpg, .gif, .bmp, .png, .pdf
        <?
		}else{
      ...
      <tr>
		...
		?>
        <input type="file" name="cwk_cert_file" id="cwk_cert_file" class="file_upload"/> เฉพาะ .jpg, .gif, .bmp, .png, .pdf
        <?
		}else{
      ...
      <tr>
       ...
       <tr>
        ...
        <img name="pic_cancel2" id="pic_cancel2" src="../images/active_default/cancel_active_buttons_03.png" border="0" onclick="load_page3('current_work.php');" style="cursor:pointer; display:none" onmouseover="over_button('pic_cancel','pic_cancel2');" onmouseout="out_button('pic_cancel','pic_cancel2')"/>
        </td></tr>
