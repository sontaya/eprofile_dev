...
$c2 = 0;
$c3 = 0;
	$sql_count = "SELECT EMP_ID FROM ".TB_CURRENT_WORK_TAB."  WHERE CWK_MUA_MAIN = '".$_REQUEST['type']."' AND CWK_STATUS <> '02' AND CWK_STATUS <> '04' AND CWK_STATUS <> '07'     ";
	...
	$p3 = 0;
	$p4 = 0;
	$all_person =  count_row(TB_CURRENT_WORK_TAB," WHERE CWK_MUA_MAIN = '".$_REQUEST['type']."' AND CWK_STATUS <> '02' AND CWK_STATUS <> '04' AND CWK_STATUS <> '07'   ",$conn); 
	if($all_person > 0){
...
$graph = new PHPGraphLibPie(500, 250);
$data = array("Doctor" => $num1_all, "Master" => $num2_all,"Bachelor" => $num3_all, "Other" =>$num4_all);
$graph->addData($data);
$graph->createGraph();
} // end if $num>0
