...
//print $check_row;
  if ($check_row > 0) {
    ...
    if ($check_row > 0) {
      $sql = "SELECT SDU_BIODATA_TAB.*,SDU_CURRENT_WORK_TAB.EMP_ID,SDU_CURRENT_WORK_TAB.CWK_STATUS,SDU_CURRENT_WORK_TAB.CWK_MUA_MAIN,SDU_CURRENT_WORK_TAB.CWK_MUA_SUBMAIN FROM SDU_BIODATA_TAB,SDU_CURRENT_WORK_TAB WHERE $q_where ORDER BY BIO_FNAME_TH ASC  ";
//echo $sql;
      $stid = oci_parse($conn, $sql);
      oci_execute($stid);
      ?>
      ...
      <table width="716"  border="0" align="center"  bgcolor="#e9e9e9" >
        ...
        while (($row = oci_fetch_array($stid, OCI_BOTH))) {
          ...
          <tr align="center" height="22" valign="top">
            ...
            <?  $stid1 = oci_parse($conn, "select * from FACULTY where CODE_FACULTY=".$row["CWK_MUA_MAIN"]."");
                   oci_execute($stid1);
				    OCIFetch($stid1);
	               ?>
                   <?
