...
<title>บัญชีถือจ่ายเงินเดือนเจ้าหน้าที่ประจำตามสัญญาจ้าง</title>
<style type="text/css">
body,td,th {
...
<body>
<div align="center">บัญชีถือจ่ายเงินเดือนเจ้าหน้าที่ประจำตามสัญญาจ้าง<br />มหาวิทยาลัยราชภัฏสวนดุสิต</div><br />
<table width="1293" border="0" cellspacing="0" cellpadding="8" align="center">
  ...
  <?
$sql =  "SELECT * FROM ".TB_CURRENT_WORK_TAB."  WHERE  CWK_STATUS = '01'  AND (CWK_MUA_EMP_TYPE <> '1') AND (CWK_DSU_POS NOT BETWEEN 15 AND 23  )  AND (CWK_DSU_POS NOT BETWEEN 123 AND 139  ) AND (CWK_DSU_POS NOT BETWEEN 77 AND 79  ) ";
...
while($row = oci_fetch_array($stid, OCI_BOTH)){
  ...
  ?>
  <tr>
    <td align="left" valign="top" class="data_dot" style="font-size:13px"><?=get_name2($row["EMP_ID"],TB_BIODATA_TAB)?><br /><?=get_person_id($row["EMP_ID"],TB_BIODATA_TAB)?></td>
    <td align="left" valign="top" class="data_dot" style="font-size:13px"><? echo get_position2($row["CWK_DSU_POS"],TB_POSITION);?></td>
    <td align="center" valign="top" class="data_dot"><? echo $row["CWK_DSU_POS"];?></td>
...
<?
	...
	?></td>
    <td align="right" valign="top" class="data_dot" style="font-size:13px;padding-right: 10px"><? 
	if(($salary[0] > $salary[1]) and ($salary[1] > 0) and ($salary[0] - $salary[1]) > 0) echo number_format(($salary[0] - $salary[1]),2);
	?></td>
    <td valign="top" class="data_dot">&nbsp;</td>
    <td valign="top" class="data_dot">&nbsp;</td>
  </tr>
  <?
  unset($salary);
...
<div align="center"> <input type="button" value="Export to Excel" onclick="window.location = 'salary_report3_excel.php?excel=1'"/></div>
