...
header("Content-Disposition: attachment; filename=salary_report_".date("d-m-").(date("Y") +543).".xls;");
...
$money1 = 0;
$money2 = 0;
$sql =  "SELECT * FROM ".TB_CURRENT_WORK_TAB."  WHERE  CWK_STATUS = '01'  AND (CWK_MUA_EMP_TYPE = '4' OR CWK_MUA_EMP_TYPE = '5' ) AND (CWK_MUA_VPOS <> '00' OR CWK_DSU_POS BETWEEN 123 AND 139 )  ";
$stid = oci_parse($conn, $sql );
oci_execute($stid);
while($row = oci_fetch_array($stid, OCI_BOTH)){
	$count_person1 = $db->count_row(TB_REF_SALARY_STEP," WHERE  EMP_ID = '".$row["EMP_ID"]."' AND  SOURCE1 <> '08' GROUP BY EMP_ID",$conn);
...
$sql =  "SELECT * FROM ".TB_CURRENT_WORK_TAB."  WHERE  CWK_STATUS = '01'  AND (CWK_MUA_EMP_TYPE = '4' OR CWK_MUA_EMP_TYPE = '5' ) AND (CWK_MUA_VPOS <> '00' OR CWK_DSU_POS BETWEEN 123 AND 139 )  AND CWK_MUA_MAIN = '".$row1["CODE_FACULTY"]."' ";
$stid = oci_parse($conn, $sql );
oci_execute($stid);
while($row = oci_fetch_array($stid, OCI_BOTH)){
	$count_person1 = $db->count_row(TB_REF_SALARY_STEP," WHERE  EMP_ID = '".$row["EMP_ID"]."' AND  SOURCE1 <> '08' GROUP BY EMP_ID",$conn);
