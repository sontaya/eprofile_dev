...
@session_start();
	...
	$numrow = $db->count_row(TB_CONSULT_COMMIT_TAB," WHERE  EMP_ID = '".$_SESSION["EMP_ID"]."'",$conn);
...
<table width="98%"  border="0" align="center"  bgcolor="#e9e9e9" >
    <tr align="center"  class="text_th">
        ...
        <td width="31%" class="text_tr">ชื่อหน่วยงานภายนอก</td>
    ...
    $sql = "SELECT * FROM  ".TB_CONSULT_COMMIT_TAB."  WHERE  EMP_ID= '".$_SESSION["EMP_ID"]."' ORDER BY COM_ID ASC"; 
	$id = 1;
    ...
    <tr align="center" height="20" valign="top">
		...
		?>
        <a href="files/consult_file/<?=$row["COM_FILE"]?>" target="_blank">
		...
		<img src="../images/b_edit.png" height="15" border="0" style="cursor:pointer" title="Edit" class="vtip"  onclick="edit_consult('<?=$id?>')"/>
		</td>
        <?php } ?>
        <?php if($_SESSION['USER_TYPE'] != 'chief' || $_SESSION['EMP_ID'] == $_SESSION['USER_EMP_ID']) { ?>
        <td align="center" valign="middle" class="text_td">
		<img src="../images/b_del.png" height="15" border="0" style="cursor:pointer" title="Delete" class="vtip"  onclick="del_consult('<?=$row["COM_ID"]?>','<?=$row["COM_FILE"]?>')"/>
        </td>
        <?php } ?>
    </tr>
    ...
    <div id="com_org_name<?=$id?>"><?=$row["COM_ORG_NAME"]?></div>
     <div id="com_course<?=$id?>"><?=$row["COM_COURSE"]?></div>
      <div id="com_detail<?=$id?>"><?=$row["COM_DETAIL"]?></div>
    ...
    <div id="debugg" style="border:1px solid #F00; display: none;">&nbsp;</div>
<br />
    <? } 
