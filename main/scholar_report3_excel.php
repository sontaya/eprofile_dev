...
header("Content-Disposition: attachment; filename=finish_graduation_v_report_".date("d-m-").(date("Y") +543).".xls;");
...
$sql_all_person1 =  "SELECT * FROM ".TB_SCHOLAR_TAB." WHERE SCH_EDU_END_DATE BETWEEN TO_DATE('$d1','YYYY-MM-DD') AND TO_DATE('$d2','YYYY-MM-DD')";
...
while($row_all_person1 = oci_fetch_array($stid_all_person1, OCI_BOTH)){
	...
	if($all_person_temp1 > 0 ){
		...
		$num1_all_temp23 =  $db->count_row(TB_CURRENT_WORK_TAB," WHERE  EMP_ID = '".$row_all_person1["EMP_ID"]."' AND   CWK_MUA_VPOS = '04' AND  CWK_MUA_EMP_TYPE = '1' AND CWK_MUA_MAIN = '04'  AND CWK_STATUS <> '02' AND CWK_STATUS <> '03'  AND CWK_STATUS <> '04' AND CWK_STATUS <> '07'   ");
...
ประเภท ข้อมูล สำเร็จการศึกษา<br />
<br />
</div>
