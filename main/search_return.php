...
$jnc_depart_sub = trim($_POST["jnc_depart_sub"]);
$cwk_status = trim($_POST["cwk_status"]);
if ($name != "") {
...
if($jnc_depart){
$q_where .= " AND SDU_CURRENT_WORK_TAB.CWK_MUA_MAIN LIKE '%{$jnc_depart}%' ";
}
if($jnc_depart_sub){
$q_where .= " AND SDU_CURRENT_WORK_TAB.CWK_MUA_SUBMAIN LIKE '%{$jnc_depart_sub}%' ";
}
if($emp_type){
$q_where .= " AND SDU_CURRENT_WORK_TAB.CWK_MUA_EMP_TYPE LIKE '%{$emp_type}%' ";
}
if($cwk_status){
$q_where .= " AND SDU_CURRENT_WORK_TAB.CWK_STATUS LIKE '%{$cwk_status}%' ";
}
//echo $q_where2;

if($jnc_depart){
$q_where2 .= " AND SDU_CURRENT_WORK_TAB.CWK_MUA_MAIN LIKE '%{$jnc_depart}%' ";
}
if($jnc_depart_sub){
$q_where2 .= " AND SDU_CURRENT_WORK_TAB.CWK_MUA_SUBMAIN LIKE '%{$jnc_depart_sub}%' ";
}
if($emp_type){
$q_where2 .= " AND SDU_CURRENT_WORK_TAB.CWK_MUA_EMP_TYPE LIKE '%{$emp_type}%' ";
}
if($cwk_status){
$q_where2 .= " AND SDU_CURRENT_WORK_TAB.CWK_STATUS LIKE '%{$cwk_status}%' ";
}
//echo $q_where2;
...
//print $check_row;
  if ($check_row > 0) {
    ...
    if ($check_row > 0) {
      $sql = "SELECT SDU_BIODATA_TAB.*,SDU_CURRENT_WORK_TAB.EMP_ID,SDU_CURRENT_WORK_TAB.CWK_DSU_EDU_CENTER,SDU_CURRENT_WORK_TAB.CWK_STATUS,SDU_CURRENT_WORK_TAB.CWK_MUA_MAIN,SDU_CURRENT_WORK_TAB.CWK_MUA_SUBMAIN FROM SDU_BIODATA_TAB,SDU_CURRENT_WORK_TAB WHERE $q_where ORDER BY BIO_FNAME_TH ASC  ";
//echo $sql;
      $stid = oci_parse($conn, $sql);
      oci_execute($stid);
      ?>
      ...
      <table width="716"  border="0" align="center"  bgcolor="#e9e9e9" >
        <tr align="center" class="text_th">
          <td width="52" class="text_tr">ลำดับ</td>
          <td width="150" class="text_tr">คำนำหน้า</td>
          <td width="150" class="text_tr">ยศ/ตำแหน่ง</td>
          <td width="150" class="text_tr">ชื่อ - นามสกุล</td>
          <td width="166" class="text_tr">หน่วยงานหลัก</td>
          <td width="166" class="text_tr">หน่วยงานย่อย</td>
          <td width="166" class="text_tr">ศูนย์การศึกษา</td>
        ...
        while (($row = oci_fetch_array($stid, OCI_BOTH))) {
          ...
          <tr align="center" height="22" valign="top">
            <td align="center" class="text_td"><?= $n ?></td>
            <td align="left" class="text_td text_data"><?= $row["BIO_TITLE_TH"] ?></td>
            <td align="left" class="text_td text_data"><?= $row["BIO_TITLE2_TH"] ?></td>
            <td align="left" class="text_td text_data"><?= $row["BIO_FNAME_TH"] ?>  <?= $row["BIO_MNAME_TH"] ?> <?= $row["BIO_LNAME_TH"] ?></td>
            <?  $stid1 = oci_parse($conn, "select * from ".TB_REF_DEPARTMENT." where CODE_FACULTY=".$row["CWK_MUA_MAIN"]."");
                   oci_execute($stid1);
				    OCIFetch($stid1);
	               ?>
                   <?
				   $sql_sub = "SELECT * FROM  ".TB_REF_DEPARTMENT_SUB."  WHERE CODE_DEPARTMENT_SECTION = '".$row["CWK_MUA_SUBMAIN"]."' and CODE_FACULTY=".$row["CWK_MUA_MAIN"].""; 
//				   echo $sql_sub;
					$stid_ref_sub = oci_parse($conn, $sql_sub);
					oci_execute($stid_ref_sub);
					OCIFetch($stid_ref_sub);
				   ?>
              <td align="left" class="text_td text_data"><?=ociresult($stid1,"NAME_FACULTY"); ?></td>
              <td align="left" class="text_td text_data"><?= ociresult($stid_ref_sub,"NAME_DEPARTMENT_SECTION"); ?></td>
					 <?
                $sql_ref_site = "SELECT * FROM  ".TB_REF_SITE." WHERE CODE_SITE='".$row["CWK_DSU_EDU_CENTER"]."' ORDER BY NAME_SITE ASC ";
//                echo $sql_ref_site;
                $stid_ref_site = oci_parse($conn, $sql_ref_site);
                oci_execute($stid_ref_site);
					OCIFetch($stid_ref_site);
                ?>
              <td align="left" class="text_td text_data"><?= ociresult($stid_ref_site,"NAME_SITE"); ?></td>
            <td align="center" class="text_td"><?= $row["EMP_ID"]?></td>
