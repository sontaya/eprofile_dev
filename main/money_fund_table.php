...
<table width="100%"  border="0" align="center"  bgcolor="#e9e9e9" >
    <tr align="center"  class="text_th">
		...
		<td width="5%" class="text_tr">จำนวนเงินทุนรวม</td>
        <td width="5%" class="text_tr">เอกสารหลักฐาน</td>
         <?php
    ...
    $sql = "SELECT * FROM  ".TB_SDU_MUNNY_TAB."  WHERE  EMP_ID= '".$_SESSION["EMP_ID"]."' ORDER BY M_ID2 ASC"; 
...
while (($row = oci_fetch_array($stid, OCI_BOTH))) {$s++
    ...
    <tr align="center" height="20" valign="top">
        ...
        <td class="text_td"><?=change_date_thai($row["DATE_START"])?></td>
        <td class="text_td"><?=change_date_thai($row["DATE_END"])?></td>
        <td class="text_td"><?=number_format( $row["NB_MONEY"] , 2 );?></td>
        <? $dd=$row["MONEY_ONE"]+$row["MONEY_TWO"]+$row["MONEY_THEE"]; ?>
        <td class="text_td"><?=number_format( $dd , 2 );?></td>
		<? $numq=$row["NB_MONEY"]+$dd; ?>
		<td class="text_td"><?=number_format($numq,2);?></td>
		<td class="text_td"><? if($row["MUNNY_FILE"]){
			echo "<a href='files/money_fund_file/".$row["MUNNY_FILE"]."' target='_blank'><img src='../images/macosx100.png' width='25'></a>";
		}?></td>
          <?php
	if($_SESSION['USER_TYPE'] != 'user') {
	?>
    ...
    <div id="munny_file<?=$id?>"><?=$row["MUNNY_FILE"]?></div>
